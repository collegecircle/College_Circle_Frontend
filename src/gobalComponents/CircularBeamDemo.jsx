...
//                             onTap={handleRedirect} // ✅ ensures mobile taps work
//                             className="relative z--1 bg-gradient-to-r from-yellow-100 to-yellow-200 text-black px-3 xs:px-4 sm:px-6 md:px-8 lg:px-10 py-2 xs:py-2.5 sm:py-3 md:py-3.5 lg:py-3 rounded-full font-bold text-xs xs:text-sm sm:text-base md:text-lg lg:text-xl hover:from-yellow-300 hover:to-yellow-400 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-yellow-400/20 focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:ring-opacity-50 active:scale-95 cursor-pointer pointer-events-auto"
//                             whileHover={{ scale: 1.05 }}
//                             whileTap={{ scale: 0.98 }}
//                             initial={{ y: 20, opacity: 0 }}
//                             animate={{ y: 0, opacity: 1 }}
...
const AnimatedBeam = ({
    ...
    return (
        <svg
            ...
            className={`pointer-events-none absolute left-0 top-0 transform-gpu stroke-2 ${className}`}
            viewBox={`0 0 ${svgDimensions.width} ${svgDimensions.height}`}
            style={{ zIndex: 1 }} // Lower z-index for beams
        >
            <path
                d={pathD}
                stroke={pathColor}
...
const CircularBeamDemo = () => {
    ...
    return (
        <div className="w-full h-screen overflow-hidden bg-slate-1900" style={{ position: 'relative', zIndex: 1 }}>
            <div
                ref={containerRef}
                className="relative w-full h-full p-2 sm:p-4 overflow-hidden"
                style={{ zIndex: 1 }}
            >
                ...
                {points.map((point, index) => (
                    ...
                    >
                        <span className={`${pointClasses.padding} ${pointClasses.text} text-white drop-shadow-md font-medium pointer-events-none`}>
                            {point.name}
                        </span>
                    </motion.div>
                ))}
