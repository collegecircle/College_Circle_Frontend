...
// import { Search, Plus, Edit, Trash2, Building, MapPin, Calendar, Eye, Globe, Users, X } from "lucide-react";

// const CollegesManagement = () => {
//   const dispatch = useDispatch();
//   const { list, status, error } = useSelector((state) => state.colleges);

//   // Safely extract actual array of colleges
//   const colleges = Array.isArray(list?.data?.colleges)
//     ? list.data.colleges
//     : [];

//   console.log("Colleges from Redux:", colleges);
...
//       }
//     });
//     setSlideImages(college.gallery?.slideImages?.length > 0 ? college.gallery.slideImages : [""]);
//     setStreams(college.streams || []);
//     setShowModal(true);
//   };
...
//     setSlideImages(slideImages.filter((_, i) => i !== index));
//   };
...
//       setStreams([...streams, { ...newStream, subStreams: newStream.subStreams || [] }]);
...
//       if (!updatedStreams[streamIndex].subStreams) {
//         updatedStreams[streamIndex].subStreams = [];
//       }
//       updatedStreams[streamIndex].subStreams.push({
//         ...newSubStream,
//         courses: newSubStream.courses || []
//       });
...
//     const updatedStreams = [...streams];
//     updatedStreams[streamIndex].subStreams = updatedStreams[streamIndex].subStreams.filter((_, i) => i !== subStreamIndex);
//     setStreams(updatedStreams);
//   };
...
//         slideImages: slideImages.filter(img => img.trim() !== "")
//       },
//       streams: streams
//     };

//     if (currentCollege) {
//       dispatch(updateCollege({ id: currentCollege.id, collegeData: payload }));
//     } else {
...
//       (college.address?.city || "").toLowerCase().includes(search.toLowerCase())
//     )
//     : [];
...
//           <div>
//             <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Colleges Management</h1>
//             <p className="text-gray-600 mt-1 text-sm sm:text-base">Manage college information and details</p>
//           </div>
//           <button
//             onClick={handleAdd}
...
//             <div key={college.id} className="p-4 border-b border-gray-200 last:border-b-0">
...
//                   <div className="text-sm text-gray-500">{formatAddress(college.address)}</div>
//                   <div className="flex items-center space-x-2 mt-2">
//                     <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${college.type === 'Private'
...
//                         <div className="text-sm text-gray-500">{college.accreditation || 'N/A'}</div>
//                       </div>
//                     </div>
//                   </td>
...
//                   {modalType === 'add' ? 'Add New College' :
//                     modalType === 'edit' ? 'Edit College' :
//                       'College Details'}
//                 </h3>
//                 <button
...
//   {/* Address */}
//   <div className="space-y-4">
//     <h4 className="text-md font-semibold text-gray-900 border-b pb-2">Address</h4>
//     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//       <div>
...
//                 {/* Other Details */}
//                 <div className="space-y-4">
//                   <h4 className="text-md font-semibold text-gray-900 border-b pb-2">College Details</h4>
//                   <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
//                     <div>
...
//                 {/* Gallery Section */}
//                 <div className="space-y-4">
//                   <h4 className="text-md font-semibold text-gray-900 border-b pb-2">Gallery</h4>
//                   <div>
...
//                       onChange={(e) => setFormData({
//                         ...formData,
//                         gallery: { ...formData.gallery, logoUrl: e.target.value }
//                       })}
//                       disabled={modalType === 'view'}
//                       className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100"
//                     />
...
//                 {/* Streams Section */}
//                 <div className="space-y-4">
//                   <h4 className="text-md font-semibold text-gray-900 border-b pb-2">Academic Streams</h4>

//                   {/* Display existing streams */}
//                   {streams.map((stream, streamIndex) => (
//                     <div key={streamIndex} className="border border-gray-200 rounded-lg p-4 space-y-3">
...
//                       {/* Sub-streams */}
//                       {stream.subStreams && stream.subStreams.map((subStream, subStreamIndex) => (
//                         <div key={subStreamIndex} className="bg-gray-50 p-3 rounded-lg">
//                           <div className="flex items-center justify-between mb-2">
//                             <span className="font-medium text-sm text-gray-700">{subStream.name}</span>
//                             {modalType !== 'view' && (
//                               <button
//                                 type="button"
//                                 onClick={() => removeSubStream(streamIndex, subStreamIndex)}
//                                 className="text-red-600 hover:text-red-800"
//                               >
//                                 <X className="w-3 h-3" />
//                               </button>
...
//                             Courses: {subStream.courses ? subStream.courses.join(", ") : "N/A"}
//                           </div>
//                         </div>
//                       ))}
...
//                               onChange={(e) => setNewSubStream({ ...newSubStream, name: e.target.value })}
//                               className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
//                             />
//                             <input
//                               type="text"
//                               placeholder="Courses (comma separated)"
//                               value={newSubStream.courses ? newSubStream.courses.join(", ") : ""}
//                               onChange={(e) => setNewSubStream({
//                                 ...newSubStream,
...
//                     >
//                       {currentCollege ? 'Update College' : 'Create College'}
//                     </button>
//                   </div>
//                 )}
//               </form>
...
const CollegesManagement = () => {
  ...
  const colleges = Array.isArray(list?.data?.colleges) ? list.data.colleges : [];
    ...
    if (newSubStream.name.trim()) {
      setStreams((prevStreams) => {
        ...
        const subStreamToAdd = {
          ...
          courses: Array.isArray(newSubStream.courses) ? [...newSubStream.courses] : [],
        };
        updatedStreams[streamIndex].subStreams.push(subStreamToAdd);
        console.log("Added subStream to stream", streamIndex, ":", subStreamToAdd);
  ...
  const removeSubStream = (streamIndex, subStreamIndex) => {
    setStreams((prevStreams) => {
      const updatedStreams = [...prevStreams];
      if (updatedStreams[streamIndex] && updatedStreams[streamIndex].subStreams) {
        updatedStreams[streamIndex].subStreams = updatedStreams[streamIndex].subStreams.filter(
          (_, i) => i !== subStreamIndex
        );
      }
      return updatedStreams;
    });
  ...
  const handleSubmit = (e) => {
    ...
    console.log("Final payload streams:", JSON.stringify(processedStreams, null, 2));
            ...
            <div key={college.id} className="p-4 border-b border-gray-200 last:border-b-0">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center space-x-2 mt-2">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${college.type === "Private" ? "bg-purple-100 text-purple-800" : "bg-green-100 text-green-800"
                        }`}
                    >
                      {college.type}
                    </span>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {colleges.map((college) => (
                <tr key={college.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{college.university}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatAddress(college.address)}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${college.type === "Private" ? "bg-purple-100 text-purple-800" : "bg-green-100 text-green-800"
                        }`}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{college.establishedYear}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex space-x-2">
                      <button
                        onClick={() => handleView(college)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[95vh] overflow-hidden flex flex-col">
            <div className="p-4 sm:p-6 border-b flex-shrink-0">
              <div className="flex justify-between items-center">
                <h3 className="text-lg sm:text-xl font-semibold text-gray-900">
                  {modalType === "add" ? "Add New College" : modalType === "edit" ? "Edit College" : "College Details"}
                </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  {streams.map((stream, streamIndex) => (
                    <div key={streamIndex} className="border border-gray-200 rounded-lg p-4 space-y-3">
                      ...
                      {stream.subStreams && Array.isArray(stream.subStreams) && stream.subStreams.length > 0 && (
                        <div className="space-y-2">
                          <h6 className="text-sm font-medium text-gray-700">Sub-streams:</h6>
                          {stream.subStreams.map((subStream, subStreamIndex) => (
                            <div key={subStreamIndex} className="bg-gray-50 p-3 rounded-lg">
                              <div className="flex items-center justify-between mb-2">
                                <span className="font-medium text-sm text-gray-700">{subStream.name}</span>
                                {modalType !== "view" && (
                              ...
                              <div className="text-xs text-gray-600">
                                Courses:{" "}
                                {subStream.courses && Array.isArray(subStream.courses) && subStream.courses.length > 0
                      ...
                      {modalType !== "view" && (
                        <div className="space-y-2 border-t border-gray-200 pt-3">
                          <h6 className="text-sm font-medium text-gray-700">Add Sub-stream to "{stream.name}":</h6>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            <input
                              type="text"
                              placeholder="Sub-stream name (e.g., Computer Science)"
                            ...
                            <input
                              type="text"
                              placeholder="Courses (comma separated)"
                              value={Array.isArray(newSubStream.courses) ? newSubStream.courses.join(", ") : ""}
                              onChange={(e) =>
                                setNewSubStream({
                                  ...newSubStream,
                                  courses: e.target.value.split(",").map((c) => c.trim()).filter(Boolean),
                  ...
                  {modalType !== "view" && (
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 space-y-3">
                      <h6 className="text-sm font-medium text-gray-700">Add New Stream:</h6>
                      <input
                        type="text"
                        placeholder="Stream name (e.g., Engineering, Management)"
                        value={newStream.name}
                ...
                {modalType !== "view" && (
                  <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-6 border-t">
                    ...
                    >
                      {status === "loading" ? "Processing..." : currentCollege ? "Update College" : "Create College"}
                    </button>
                  </div>
                )}
              </form>
