...
import { Search, Plus, Edit, Trash2, Building, MapPin, DollarSign, Clock, Calendar, ChevronLeft, ChevronRight } from "lucide-react";

const JobsManagement = () => {
    const dispatch = useDispatch();
    const { list, status, error: reduxError } = useSelector((state) => state.jobs);
    ...
    const handleSubmit = (e) => {
        ...
        dispatch(currentJob ? updateJob({ id: currentJob.id, jobData: jobPayload }) : createJob(jobPayload))
            .then((result) => {
                if (result.type.endsWith("/fulfilled")) {
                    handleAdd();
                    fetchJobsWithPagination(currentPage, search);
        ...
        <div className="w-full p-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    <div className="hidden md:flex bg-gray-100 rounded-lg p-1">
                        <button
                            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors duration-200 ${viewMode === "table" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"
                                }`}
                            onClick={() => setViewMode("table")}
                        >
                            Table
                        ...
                        <button
                            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors duration-200 ${viewMode === "cards" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"
                                }`}
                            onClick={() => setViewMode("cards")}
                        >
                            Cards
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {jobs.map((job) => (
                                                <tr key={job.id} className="hover:bg-gray-50 transition-colors duration-200">
                                                    ...
                                                    <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                                                        <span
                                                            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${job.status === "Open" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                                                                }`}
                                                        >
                                                            {job.status || "Unknown"}
                                                        </span>
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                    <button
                                        ...
                                        className={`px-3 py-1 border border-gray-300 rounded-lg text-sm font-medium ${currentPage === page ? "bg-blue-600 text-white" : "text-gray-700 hover:bg-gray-50"
                                            }`}
                                    >
                                        {page}
                                    </button>
            ...
            <div className="mt-10 bg-white border rounded-lg p-3 sm:p-6 shadow-sm">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                        ...
                        >
                            {status === "loading" ? "Processing..." : currentJob ? "Update Job" : "Create Job"}
                        </button>
                        <button
                            type="button"
                            onClick={handleAdd}
