...
const JobsManagement = () => {
  ...
  // Extract jobs and pagination info from API response
  const jobs = Array.isArray(list?.data?.data?.jobs)
    ? list.data.data.jobs
    : Array.isArray(list?.data?.jobs)
      ? list.data.jobs
      : [];
  const totalCount = list?.data?.data?.count || list?.data?.count || 0;
  const totalPages = Math.ceil(totalCount / 5);
  const itemsPerPage = 5;
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this job?")) {
      ...
      if (deleteJob.fulfilled.match(result)) {
        ...
        fetchJobsWithPagination(currentPage, search);
      } else if (deleteJob.rejected.match(result)) {
        setError(result.payload || "Failed to delete job. Please try again.");
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    } else if (createJob.rejected.match(result) || updateJob.rejected.match(result)) {
      setError(result.payload || "Failed to save job. Please try again.");
    }
  };
  ...
  const JobCard = ({ job }) => (
    <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4 shadow-sm hover:shadow-md transition-shadow duration-200">
      <div className="flex items-start justify-between mb-3">
        ...
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium flex-shrink-0 ${job.status === "Open"
            ? "bg-green-100 text-green-800"
            : "bg-red-100 text-red-800"
            }`}
        >
      ...
      <div className="flex items-center justify-end space-x-2">
        <button
          className="flex items-center px-2 py-1 sm:px-3 sm:py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs sm:text-sm rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          onClick={() => handleEdit(job)}
        ...
        <button
          className="flex items-center px-2 py-1 sm:px-3 sm:py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs sm:text-sm rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          onClick={() => handleDelete(job.id)}
          disabled={isDeletingId === job.id}
        >
          {isDeletingId === job.id ? (
          ...
          {isDeletingId === job.id ? "Deleting..." : "Delete"}
        </button>
      </div>
    </div>
  );
    ...
    <div className="w-full p-4">
      ...
      {reduxError && status === "failed" && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start">
          ...
          <div className="flex-1">
            ...
            <p className="text-red-700 text-sm mt-1">{reduxError}</p>
          </div>
          <button
            onClick={() => fetchJobsWithPagination(currentPage, search)}
            className="text-red-600 hover:text-red-800 ml-2 text-sm font-medium"
          >
            Retry
          </button>
        </div>
      ...
      {status === "loading" && !jobs.length ? (
        <div className="flex flex-col justify-center items-center py-20">
          ...
          <span className="text-gray-500 text-sm mt-1">Please wait</span>
        </div>
      ) : (
        <>
          <div className="md:hidden">
            {jobs.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  {search ? "Try adjusting your search" : "Add a job to get started"}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                      ...
                      {jobs.map((job) => (
                        ...
                        >
                          ...
                          <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex justify-end space-x-2">
                              <button
                                className="inline-flex items-center px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                onClick={() => handleEdit(job)}
                                disabled={isDeletingId === job.id}
                              >
                                <Edit className="w-3 h-3 mr-1" />
                              ...
                              <button
                                className="inline-flex items-center px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                onClick={() => handleDelete(job.id)}
                                disabled={isDeletingId === job.id}
                              >
                                {isDeletingId === job.id ? (
          ...
          {totalPages > 1 && (
            <div className="px-4 sm:px-6 py-3 bg-gray-50 border-t flex flex-col sm:flex-row justify-between items-center gap-3 mt-6 rounded-lg">
              <div className="text-sm text-gray-700 text-center sm:text-left">
                Showing {(currentPage - 1) * itemsPerPage + 1} to{" "}
                {Math.min(currentPage * itemsPerPage, totalCount)} of{" "}
                {totalCount} results
                ...
                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                  ...
                  return (
                    <button
                      key={page}
                      onClick={() => handlePageChange(page)}
                      className={`px-3 py-1 border border-gray-300 rounded-lg text-sm font-medium ${currentPage === page
      ...
      <div id="job-form" className="mt-10 bg-white border rounded-lg p-3 sm:p-6 shadow-sm">
        <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-4 sm:mb-6">
          {currentJob ? "Edit Job" : "Add New Job"}
        </h2>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
            <button
              type="submit"
              className="w-full sm:flex-1 px-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white text-sm sm:text-base font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
              disabled={isSubmitting}
            >
              {isSubmitting ? (
                <>
                  <Loader className="w-4 h-4 mr-2 animate-spin" />
                  {currentJob ? "Updating..." : "Creating..."}
                </>
              ) : (
                <>{currentJob ? "Update Job" : "Create Job"}</>
              )}
            ...
            <button
              type="button"
              onClick={handleAdd}
              className="w-full sm:w-auto px-4 py-2.5 bg-gray-300 hover:bg-gray-400 text-gray-800 text-sm sm:text-base font-medium rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isSubmitting}
            >
              Reset
            </button>
