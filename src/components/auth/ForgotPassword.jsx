...
//   return (
//     <div
//       className={`fixed top-4 right-4 z-50 p-4 rounded-lg border ${bg} ${text} shadow-lg max-w-sm animate-slide-in`}
//     >
...
//             {step === 1 && (
//               <Mail className="w-12 h-12 text-yellow-600 mx-auto mb-2" />
//             )}
//             {step === 2 && (
...
import { forgotPassword, resetPassword, resetMessages } from "../adminDashboard/authSlice";
...
function Notification({ type = "success", message, onClose }) {
  ...
  return (
    <div className={`fixed top-4 right-4 z-50 p-4 rounded-lg border ${bg} ${text} shadow-lg`}>
      <div className="flex items-center gap-3">
...
export default function ForgotPassword() {
  ...
  const { loading, error, success, forgotEmail } = useSelector(state => state.auth);
  ...
  const handleEmailSubmit = async (e) => {
    ...
    await dispatch(forgotPassword({ email })).unwrap()
      .then(() => setStep(2))
      .catch(() => { }); // errors handled by notification
  };

  const handlePasswordSubmit = async (e) => {
    e.preventDefault();
    if (!otp) return showNotification("error", "OTP is required");
    if (!password || !confirmPwd) return showNotification("error", "Both password fields are required");
    if (password !== confirmPwd) return showNotification("error", "Passwords do not match");

    await dispatch(resetPassword({ email: forgotEmail || email, password, otp })).unwrap()
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 flex flex-col lg:flex-row">
      ...
      {/* Branding Section */}
      <div className="w-full lg:w-1/2 bg-black flex flex-col items-center justify-center p-8 lg:p-12 text-white">
        <img src={logo} alt="Logo" className="w-48 h-48 object-contain mb-6" />
        <h2 className="text-3xl font-bold mb-2 uppercase">Forgot Password?</h2>
        <p className="text-gray-300 text-center">We’ll send you a code to reset your password.</p>
      </div>

      {/* Form Section */}
      <div className="w-full lg:w-1/2 bg-white flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          <div onClick={() => navigate("/login")} className="flex items-center gap-2 mb-8 text-yellow-600 cursor-pointer">
            <ArrowLeft size={18} />
          ...
          {step === 1 && (
            <form onSubmit={handleEmailSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 text-black py-3 px-4 rounded-full font-semibold hover:scale-[1.02] transition-all duration-200 shadow-lg disabled:opacity-50"
              >
          ...
          {step === 2 && (
            <form onSubmit={handlePasswordSubmit} className="space-y-6">
              <div className="relative">
                <Shield className="absolute left-3 top-3.5 text-gray-400" />
                <input
                  type="text"
