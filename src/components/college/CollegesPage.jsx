...
//     const dispatch = useDispatch();
//     const { list, status, error } = useSelector((state) => state.colleges);
//     console.log("Colleges Redux State:", { list, status });

//     const [activeCityFilter, setActiveCityFilter] = useState("All Cities");
//     const [activeStreamFilter, setActiveStreamFilter] = useState("All Streams");
//     const [selectedCollege, setSelectedCollege] = useState(null);
//     const [showInquiryModal, setShowInquiryModal] = useState(false);
//     const [currentPage, setCurrentPage] = useState(1);
...
//         return ["All Cities", ...(list?.data?.cities || [])];
//     }, [list?.data?.cities]);

//     // 🟡 Available streams
//     const availableStreams = useMemo(() => {
//         return ["All Streams", ...(list?.data?.streams || [])];
//     }, [list?.data?.streams]);
...
//     const fetchCollegesWithPagination = (
//         page = 1,
//         cityFilter = activeCityFilter,
//         streamFilter = activeStreamFilter
//     ) => {
//         const params = {
//             page,
//             limit: itemsPerPage,
//             city: cityFilter === "All Cities" ? "" : cityFilter,
//             stream: streamFilter === "All Streams" ? "" : streamFilter,
//         };
//         dispatch(fetchColleges(params));
//     };
//     // ✅ Fixed useEffect: no more infinite loop
//     useEffect(() => {
//         fetchCollegesWithPagination(currentPage, activeCityFilter);

//         // only run when page/city changes
//     }, [currentPage, activeCityFilter]);

//     const handleCityFilterChange = (city) => {
//         setActiveCityFilter(city);
//         dispatch(fetchColleges({ page: 1, limit: 6, city, stream: activeStreamFilter }));
//     };

//     const handleStreamFilterChange = (stream) => {
//         setActiveStreamFilter(stream);
//         dispatch(fetchColleges({ page: 1, limit: 6, city: activeCityFilter, stream }));
//     };
...
//                     <p className="text-gray-400 mb-4">{error}</p>
//                     <button
//                         onClick={() => fetchCollegesWithPagination(currentPage, activeCityFilter)}
//                         className="bg-yellow-500 text-black px-6 py-3 rounded-xl font-semibold hover:bg-yellow-600 transition-colors"
//                     >
//                         Try Again
//                     </button>
...
const CollegesPage = () => {
  ...
  const [selectedCollege, setSelectedCollege] = useState(null);
  const [showInquiryModal, setShowInquiryModal] = useState(false);
  const [showOverviewModal, setShowOverviewModal] = useState(false); // New state for overview modal
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 6;
  ...
  const handleCityFilterChange = (city) => {
    setActiveCityFilter(city);
    dispatch(
      fetchColleges({ page: 1, limit: 6, city, stream: activeStreamFilter })
    );
  };
  ...
  const handleStreamFilterChange = (stream) => {
    setActiveStreamFilter(stream);
    dispatch(
      fetchColleges({ page: 1, limit: 6, city: activeCityFilter, stream })
    );
  };
  ...
  // Handle Know More (Inquiry Modal)
  const handleKnowMore = (college) => {
    setSelectedCollege(college);
    setShowInquiryModal(true);
  };

  // Handle Overview (Overview Modal)
  const handleOverview = (college) => {
    setSelectedCollege(college);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-black to-black">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {collegesList.map((college) => (
                <CollegeCard
                  ...
                  college={college}
                  onKnowMore={() => handleKnowMore(college)}
                  onOverview={() => handleOverview(college)}
                />
              ))}
            </div>
            {totalPages > 1 && (
      ...
      {/* Overview Modal */}
      {showOverviewModal && selectedCollege && (
        <OverviewModal
          college={selectedCollege}
